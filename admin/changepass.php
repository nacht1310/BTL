...
<?php 
    function changepass($pass_old, $pass_new, $pass_confirm) {
        ...
        if(empty($pass_old) || empty($pass_new) || empty($pass_confirm)) {
            echo "<script>swal('The old password or new password or confirm password can not be empty. Please enter them correctly')</script>";
        }
        else {
            ...
            if($check == false) {
                    echo "<script>swal('The old password you enter is not correct')</script>";
            }
            elseif($pass_confirm != $pass_new) {
                    echo "<script>swal('The confirm password you enter is the same as the new password')</script>";
            }
            else {
                    $query = "UPDATE tbl_admin SET adminpass = '$pass_new' WHERE adminuser = '$user'"; 
                    $result = $db->update($query);
                    if($result != false) {
                        echo "<script>swal('The password is changed successfully')</script>";
                    }
                    else {
